...
## Repository Management & Organization Migration

Use the following terminal commands to link your local codebase to your GitHub organization, update its remotes, or re-initialize tracking.

### Link an Existing Local Repository to Your GitHub Organization

If you have already created an empty repository inside your GitHub organization dashboard, run these commands inside your local project root to point your code to it:

```bash
# 1. Verify your current remote targets
git remote -v

# 2. Change the target URL to point to your new organization
#    Replace YOUR_ORGANIZATION and YOUR_REPO_NAME with your actual values
git remote set-url origin https://github.com/YOUR_ORGANIZATION/YOUR_REPO_NAME.git

# 3. Verify the link was updated successfully
git remote -v

# 4. Push your existing code and history to the organization main branch
git push -u origin main
```

### Initialize a Fresh Project From Scratch

If you are initializing this folder structure locally for the first time:

```bash
# 1. Initialize git in your root directory
git init

# 2. Stage all project files (respecting your .gitignore)
git add .

# 3. Create your baseline commit
git commit -m "Initial commit: Set up anti-spoofing platform architecture"

# 4. Ensure your default branch is named 'main'
git branch -M main

# 5. Link it to your organization's empty repository
#    Replace YOUR_ORGANIZATION and YOUR_REPO_NAME with your actual values
git remote add origin https://github.com/YOUR_ORGANIZATION/YOUR_REPO_NAME.git
